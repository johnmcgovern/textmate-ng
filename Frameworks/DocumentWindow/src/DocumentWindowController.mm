...
// The registry and the disposable-document test used to be file statics in an
// anonymous namespace. They are class methods now (see
// DocumentWindowControllerPrivate.h) because OakDocumentControllerWindows.mm
// needs them too, and a second translation unit cannot see a file static. These
// three wrappers keep the ~15 call sites below reading as they did.
static NSMutableDictionary<NSUUID*, DocumentWindowController*>* AllControllers ()
{
	return DocumentWindowController.allControllers;
}

static NSArray<DocumentWindowController*>* SortedControllers ()
{
	return DocumentWindowController.sortedControllers;
}

static bool is_disposable (OakDocument* doc)
{
	return [DocumentWindowController isDisposableDocument:doc];
}
	...
	for(NSNumber* flag in @[ @NO, @YES ])
	{
		for(NSWindow* window in [NSApp orderedWindows])
		{
			if([window isMiniaturized] == flag.boolValue && [window.delegate respondsToSelector:@selector(identifier)])
			{
				...
				if(id controller = DocumentWindowController.allControllers[delegate.identifier])
...
+ (BOOL)isDisposableDocument:(OakDocument*)doc
{
...
static NSTouchBarItemIdentifier kTouchBarCustomizationIdentifier = @"com.j23software.TextMate-NG.touch-bar.customization-identifier";
static NSTouchBarItemIdentifier kTouchBarTabNavigationIdentifier = @"com.j23software.TextMate-NG.touch-bar.tab-navigation";
static NSTouchBarItemIdentifier kTouchBarNewTabItemIdentifier    = @"com.j23software.TextMate-NG.touch-bar.new-tab";
static NSTouchBarItemIdentifier kTouchBarQuickOpenItemIdentifier = @"com.j23software.TextMate-NG.touch-bar.quick-open";
static NSTouchBarItemIdentifier kTouchBarFindItemIdentifier      = @"com.j23software.TextMate-NG.touch-bar.find";
static NSTouchBarItemIdentifier kTouchBarFavoritesItemIdentifier = @"com.j23software.TextMate-NG.touch-bar.favorites";
