...
// MARK: - The signing tool and the app agree

// A signature produced by **bin/update-sign** must verify with the app's own
// verifier. Nothing else checks that: the tool has its own self-test and the
// verifier has its own pins, and each could be internally consistent while the
// two disagree about a format. A change from X9.63 to DER on either side, or a
// change of algorithm, is silent everywhere except here.
//
// The fixture below was generated by the tool on 2026-09-08 with a throwaway key
// that was deleted immediately after:
//
//     bin/update-sign create-key --label <uuid>   -> the public key
//     bin/update-sign sign payload.json --label … -> the signature
//     bin/update-sign delete-key --label <uuid>
//
// Only the public half is here, so there is nothing secret in it and nothing to
// rotate. Regenerate the same way if the manifest format changes.

static NSString* const kToolPublicKeyBase64 = @"BPe285K/Dq8bMCyJpSnqD0aznUxnaAENDHCMHOFxdNz/kM9S1tEx1Y2jk3CsDIrmtSyp50NEma2Ncjv1fF1fwxY=";
static NSString* const kToolSignatureBase64 = @"MEQCIBkABzCZ6Ct1BCDrWKglNMsadp9OFirD0Fc/2QQz4dwxAiBINhxTjIidiyYWl6fPegfyFCVL2/geGHrVHJgFyxQCXA==";
static NSString* const kToolSignedPayload   = @"{\"version\":\"2026.9-alpha.22\",\"sha256\":\"deadbeef\"}";

void test_a_signature_from_the_signing_tool_verifies ()
{
	SecKeyRef publicKey = [OakDownloadManager publicKeyFromBase64X963String:kToolPublicKeyBase64];
	OAK_ASSERT(publicKey != NULL);

	NSData* signature = [[NSData alloc] initWithBase64EncodedString:kToolSignatureBase64 options:0];
	NSData* payload   = [kToolSignedPayload dataUsingEncoding:NSUTF8StringEncoding];
	OAK_ASSERT(signature != nil);

	OAK_ASSERT_EQ((bool)[OakDownloadManager.sharedInstance data:payload hasValidECDSASignature:signature usingPublicKey:publicKey], true);
}

// And the same fixture must be rejected when the payload differs by one byte —
// otherwise the test above would pass against a verifier that checks nothing.
void test_the_tools_signature_does_not_cover_a_different_payload ()
{
	SecKeyRef publicKey = [OakDownloadManager publicKeyFromBase64X963String:kToolPublicKeyBase64];
	NSData* signature   = [[NSData alloc] initWithBase64EncodedString:kToolSignatureBase64 options:0];
	NSData* different   = [@"{\"version\":\"2026.9-alpha.23\",\"sha256\":\"deadbeef\"}" dataUsingEncoding:NSUTF8StringEncoding];
