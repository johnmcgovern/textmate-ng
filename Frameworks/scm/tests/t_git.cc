...
struct setup_t
{
	setup_t (std::string const& cmd)
	{
		...
		// `init -b master`: the fixture asserts the branch name, and `git init` alone
		// takes it from init.defaultBranch, which is "main" on a modern machine.
		std::string const script = text::format("{ cd '%1$s' && '%2$s' init -b master && '%2$s' config user.email 'test@example.com' && '%2$s' config user.name 'Test Test' && '%2$s' config commit.gpgsign false && touch .dummy && '%2$s' add .dummy && '%2$s' commit .dummy -mGetHead && %3$s ; } >/dev/null", jail.path().c_str(), git.c_str(), cmd.c_str());
