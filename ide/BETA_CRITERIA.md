...
| 4 | The full suite, the sanitizers and the fuzzer are green on the tagged commit | The Sanitizers workflow on that commit, not merely on `master` | **Met on alpha.33's tagged commit, 2026-09-23** — Sanitizers run 35890140273 on 47e2d153, dispatched against the tag: suite green under ASan and UBSan, then all five fuzz targets at 120 s each with no findings (settings: 280,617 runs). Still also runs weekly |
